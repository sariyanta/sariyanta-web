		...
		<>
			...
			<Box as="main">
				...
				<Flex as="section" direction="column" maxWidth="60rem" mx="auto" py="12">
					...
					<Text as="p" fontSize="1.75rem" mb="2.5rem">
				...
				<Box maxWidth={"60rem"} mx="auto" py="10">
					<Heading as="h2" mb={"6"}>
						Favourite technologies at the moment
					</Heading>
					<Grid gap={8} position="relative" templateColumns={"repeat(2, 1fr)"}>
						<Technology
							colSpan={1}
							icon={FaWordpress}
							title="WordPress"
							description="The web framework powering more than a third of the websites."
						/>
						<Technology
							colSpan={1}
							icon={FaHubspot}
							title="HubSpot"
							description="It's a simple to use yet powerful CMS."
						/>
						<Technology
							colSpan={1}
							name="nextjs"
...
function Technology({ title, description, icon, name, ...rest }) {
	return (
		<GridItem {...rest}>
			{icon ? (
				<Icon as={icon} height={8} width={8} mb="4" />
			) : (
				<Icon name={name} height={8} width={8} mb="4" />
			)}

			<Heading fontSize="1.5rem" as="h3" fontWeight={"normal"}>
				{title}
			</Heading>
			<Text fontSize="1.25rem" fontWeight={"light"} as="div">
				{description}
			</Text>
		</GridItem>
	);
