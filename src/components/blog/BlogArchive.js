    ...
    <Link href={url}>
      ...
      { post.featuredimage ?
          <div className="relative overflow-hidden rounded-t-lg w-full">
            <Image src={post.featuredimage} alt={`Picture of ${post.title}`}  width={600} height={400} className="group-hover:scale-110  transition-all ease-in duration-300"/>
          </div>
          : null
        }
        <div className="p-6 flex flex-col justify-between grow">
          ...
          <span className="uppercase text-gray-600 font-bold">{post.category}</span>
